...
export class CreateInitialTables1737129600000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const hasJobsIndex = jobsTable?.indices.some((idx) => idx.name === "IDX_jobs_is_active");
    ...
    const hasExecutionsForeignKey = executionsTable?.foreignKeys.some(
      (fk) => fk.columnNames.includes("job_id") && fk.referencedTableName === "jobs",
    );
    ...
    const hasNotificationLogsForeignKey = notificationLogsTable?.foreignKeys.some(
      (fk) => fk.columnNames.includes("job_id") && fk.referencedTableName === "jobs",
    );
